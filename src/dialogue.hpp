...
public:
  Dialogue(const string &speaker, const string &dialogueText,
  ...
  StoryDialogues FileParser() {
    ...
    while (getline(ifs, line)) {
      ...
      if (line[0] == '[' && line[line.length() - 1] == ']') {
        ...
        if (readingDialogue && !currentSpeaker.empty() &&
            !dialogueText.empty()) {
...
private:
  ...
  string WrapText(const string &text, Font font, float fontSize,
                  float maxWidth) {
    string result;
    string currentLine;
    ...
    while (words >> word) {
      ...
      float lineWidth =
          MeasureTextEx(font, testLine.c_str(), fontSize + 30, 2).x;
...
public:
  ...
  void StartDialogue(const string &filename) {
    ...
    if (!dialogues.empty()) {
      ...
      currentWrappedText = WrapText(currentDialogue->getDialogueText(), font,
                                    FONT_SIZE, wrapWidth);
      charTimer = 0.0f;
      visibleChars = 0;
      textFullyRevealed = false;
    ...
    // Handle ENTER key
      ...
      if (!textFullyRevealed) {
        ...
        if (currentDialogue == dialogues.end()) {
          ...
          float wrapWidth = boxDest.width - 40;
          currentWrappedText = WrapText(currentDialogue->getDialogueText(),
                                        font, FONT_SIZE, wrapWidth);
          charTimer = 0.0f;
          visibleChars = 0;
          textFullyRevealed = false;
        }
  ...
  void Draw() {
    ...
    Texture2D avatarTexture =
        GetAvatarTexture(currentDialogue->getAvatarPath());

    // Draw background
    DrawRectangle(0, 700, 1920, 500, BLACK);
    DrawTexturePro(boxTexture,
                   {0, 0, (float)boxTexture.width, (float)boxTexture.height},
    ...
    // Draw speaker name
    DrawTextEx(font, currentDialogue->getSpeaker().c_str(), namePos, FONT_SIZE,
               2, nameColor);
    ...
    if (showingChoices) {
      ...
      for (const auto &c : choices) {
        ...
        DrawTextEx(font, choiceDisplay.c_str(), choicePos, FONT_SIZE - 5, 2,
                   GOLD);
        yOffset += 40;
        choiceNum++;
      }
      ...
      if (textFullyRevealed) {
        DrawTextEx(font, "Press ENTER...", {1150, 980}, FONT_SIZE - 10, 2,
                   GRAY);
      } else {
        DrawTextEx(font, "Press ENTER to skip", {1100, 980}, FONT_SIZE - 10, 2,
                   DARKGRAY);
      }
    }
  }
