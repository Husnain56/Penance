...
public:
  // ========== CONSTRUCTOR ==========
  MainMenu()
      : textureLoaded(false), overlayLoaded(false), startClicked(false),
        optionsClicked(false), creditsClicked(false), exitClicked(false),
        showOptionsMenu(false), showCreditsMenu(false) {
  ...
  void Draw(bool showDebug = true) {
    ...
    // Draw background
    Rectangle bgSource = {0, 0, (float)menuBackground.width,
    ...
    // Draw overlay
    if (overlayLoaded) {
      Rectangle overlaySource = {0, 0, (float)menuOverlay.width,
                                 (float)menuOverlay.height};
      ...
      Rectangle overlayDest = {overlayX, overlayY, MENU_OVERLAY_WIDTH,
                               MENU_OVERLAY_HEIGHT};
      DrawTexturePro(menuOverlay, overlaySource, overlayDest, {0, 0}, 0.0f,
                     WHITE);
    }
  ...
  // ========== SETTERS ==========
  void SetButtonPositions(Rectangle start, Rectangle options, Rectangle credits,
                          Rectangle exit) {
    startButton = start;
